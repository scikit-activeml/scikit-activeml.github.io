...
# %%
# **Idea:** The Randomized-Variable-Uncertainty query strategy queries labels based on the classifier's uncertainty assessed based on the classifier's predictions. The sample is queried when the probability of the most likely class exceeds a time-dependent threshold calculated based on the budget, and the number of observed and acquired samples. The threshold is randomized by being multiplied with a random number sampled from N(1,delta).
...
#    :target: https://colab.research.google.com/github/scikit-activeml/scikit-activeml.github.io/blob/gh-pages/development/generated/sphinx_gallery_notebooks//4-stream/plot-RandomVariableUncertainty-Randomized-Variable-Uncertainty.ipynb
#
