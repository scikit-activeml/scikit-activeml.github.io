...
# %%
# **Idea:** Implementation of a wrapper class for pool-based active learning query strategies with a single annotator such that it transforms the query strategy for the single annotator into a query strategy for multiple annotators by choosing an annotator randomly or according to the parameter `A_perf` and setting the labeled matrix to a labeled vector by an aggregation function, e.g., majority voting. Here the single-annotator strategy is 'Core Set' and for the chosen sample the current 3 annotators with best estimated performance are chosen to provide a label.
...
#    :target: https://colab.research.google.com/github/scikit-activeml/scikit-activeml.github.io/blob/gh-pages/development/generated/sphinx_gallery_notebooks//3-pool-multi-annotator/plot-SingleAnnotatorWrapper-Core_Set_+_Greedy_Selection.ipynb
#
...
qs = SingleAnnotatorWrapper(strategy=CoreSet(random_state=random_state), random_state=random_state)
...
for c in range(n_cycles):
    ...
    query_idx = qs.query(X=X, y=y, batch_size=3, n_annotators_per_sample=3, A_perf=A_perf_clf)
    ...
    _, utilities = qs.query(
        X=X, y=y, batch_size=3, n_annotators_per_sample=3, A_perf=A_perf_clf, return_utilities=True, candidates=mesh_samples
    )
    ax1.contourf(
        X_mesh,
        Y_mesh,
