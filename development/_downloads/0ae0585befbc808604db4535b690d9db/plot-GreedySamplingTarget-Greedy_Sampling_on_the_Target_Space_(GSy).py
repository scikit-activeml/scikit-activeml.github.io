...
# %%
# **Idea:** Greedy Sampling on the Target Space (GSy) at first selects those samples that to maximize the diversity in the feature space and then selects samples to maximize the diversity in the target space. The diversity is maximized by selecting those samples that are the furthest away from all previously labeled samples. To measure the distance in the target space the strategy predicts the label corresponding to each sample.
...
#    :target: https://colab.research.google.com/github/scikit-activeml/scikit-activeml.github.io/blob/gh-pages/development/generated/sphinx_gallery_notebooks//2-pool-regression/plot-GreedySamplingTarget-Greedy_Sampling_on_the_Target_Space_(GSy).ipynb
#
