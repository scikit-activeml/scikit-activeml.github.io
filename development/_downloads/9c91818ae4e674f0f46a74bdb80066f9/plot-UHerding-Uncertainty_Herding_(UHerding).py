...
# %%
# **Idea:** UHerding greedily selects a batch by maximizing uncertainty-weighted coverage in feature space. It combines predictive uncertainty from a classifier with a kernel-based coverage objective and adapts the temperature scaling and kernel radius from the currently labeled data. Logits are preferred because they enable temperature scaling directly; if they are unavailable, UHerding falls back to the classifier's probability estimates.
...
#    :target: https://colab.research.google.com/github/scikit-activeml/scikit-activeml.github.io/blob/gh-pages/development/generated/sphinx_gallery_notebooks//1-pool-classification/plot-UHerding-Uncertainty_Herding_(UHerding).ipynb
#
...
from skactiveml.pool import UHerding
...
qs = UHerding(normalize_samples=False)
...
for c in range(n_cycles):
    ...
    ax.scatter(
        X[:, 0], X[:, 1], c=y_pool, cmap="coolwarm", marker=".", zorder=2
    )
    ax.scatter(
        X_labeled[:, 0],
        X_labeled[:, 1],
...
# .. rubric:: References:
#
# The implementation of this strategy is based on :footcite:t:`bae2025uncertainty`.
#
# .. footbibliography::
