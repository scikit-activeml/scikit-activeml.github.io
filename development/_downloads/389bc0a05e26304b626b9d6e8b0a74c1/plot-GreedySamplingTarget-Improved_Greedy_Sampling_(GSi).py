...
# %%
# **Idea:** Improved Greedy Sampling (GSi) at first selects those samples that maximize the diversity in the feature space and then selects samples to maximize the diversity in the combined space of features and targets. The diversity is maximized by selecting those samples that are the furthest away from all previously labeled samples. The distance in the combined space is measured as the product of the distance in the feature space and the target space. To measure the distance in the target space the strategy predicts the label corresponding to each unlabeled sample.
...
#    :target: https://colab.research.google.com/github/scikit-activeml/scikit-activeml.github.io/blob/gh-pages/development/generated/sphinx_gallery_notebooks//2-pool-regression/plot-GreedySamplingTarget-Improved_Greedy_Sampling_(GSi).ipynb
#
...
reg = NICKernelRegressor(
    random_state=random_state, metric_dict={"gamma": 15.0}
)
