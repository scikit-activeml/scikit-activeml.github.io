...
# %%
# **Idea:** selects `batch_size` unlabeled points to maximize empirical coverage under a fixed radius `delta` in the embedding space, treating points within `delta` of any labeled sample as already covered and greedily adding the candidate samples that covers the most new samples at each step. It chooses `delta` via a purity criterion estimated from unlabeled data, prioritizes dense regions, and does not use predictive uncertainty.
...
#    :target: https://colab.research.google.com/github/scikit-activeml/scikit-activeml.github.io/blob/gh-pages/development/generated/sphinx_gallery_notebooks//1-pool-classification/plot-ProbCover-Probability_Coverage_(ProbCover).ipynb
#
...
qs = ProbCover(
    n_classes=2, random_state=0, cluster_algo_dict={"random_state": 0}
)
